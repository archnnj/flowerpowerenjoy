	...
	\section{Introduction}
		...
		\subsection{Goals}
			...
			\begin{itemize}
				...
				\item \textbf{G2} If a user returns a car to a charging safe area and plugs it into the power grid once left, the system applies a 30\% discount on the standard price of his current ride.
				\item \textbf{G3} The system applies a 30\% increase of the standard price for the current ride if the car is returned in a safe area at more than 3 Km from the nearest charging safe area. % FIXME not clear
		...
		\subsection{Glossary}
			\begin{description}
				...
				\item[Recharging areas] are parking slots where the car can be recharged; safe areas and recharging areas do not always coincide %??????????????
